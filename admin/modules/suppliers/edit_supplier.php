...
if ($_POST) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT id FROM suppliers WHERE name = ? AND id != ? AND status = 'active'");
        ...
        if (!empty($email)) {
            $stmt = $pdo->prepare("SELECT id FROM suppliers WHERE email = ? AND id != ? AND status = 'active'");
            $stmt->execute([$email, $supplierId]);
            if ($stmt->fetch()) {
        ...
        $stmt = $pdo->prepare("
            UPDATE suppliers SET name = ?, contact_person = ?, email = ?, phone = ?, address = ?, status = ?, updated_at = NOW()
            WHERE id = ?
        ");
        $stmt->execute([$name, $contact_person, $email, $phone, $address, $status, $supplierId]);
...
<head>
    <title>Edit Supplier - <?php echo htmlspecialchars($supplier['name']); ?></title>
...
<body class="pc-shell">
    ...
    <div class="pc-container">
        <div class="pc-page-header pc-animate">
            <div class="pc-breadcrumb">Home <i class="fas fa-chevron-right"></i> <a href="index.php" class="text-blue-600 hover:underline">Suppliers</a> <i class="fas fa-chevron-right"></i> Edit</div>
            <div class="flex justify-between items-center gap-4">
                <div>
                    <h1 class="pc-page-title">Edit Supplier</h1>
                    <p class="pc-page-subtitle">Update supplier information</p>
                </div>
                ...
                <div class="section-content">
                    ...
                    <div class="form-group">
                        ...
                        <div class="icon-input">
                            ...
                            <textarea name="address" class="form-input form-textarea" placeholder="Enter full address"><?php echo htmlspecialchars($supplier['address'] ?? ''); ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
